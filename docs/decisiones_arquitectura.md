...
## 22 de agosto de 2026 (continuación) — Diseño del grafo de despacho interno de Upgrade & Review Center; se empiezan a escribir los 17 prompts de sub-bots (ClickUp `86bbhawuq`)

Con el loop Efadam → Tech center → Técnico jefe confirmado de punta a punta, se retoma el siguiente trabajo grande de la lista de pendientes: escribir los prompts de Upgrade & Review Center. `roster_agentes_v4.xlsx` (que estaba modificado y sin revisar desde una ronda anterior — se verificó que es trabajo legítimo y coherente, no un archivo a medio corromper; se commiteó tal cual) ya trae las 17 filas de sub-bots del departamento con su Input/Output orientativo. El prompt del bot cabeza (`Upgrade & review center`) ya estaba redactado en `docs/vision/Efadam/Upgrade & Review center/Upgrade & Review center.md`, pero nunca se migró a la plantilla nueva ni se copió a `prompts/` — se hace en esta ronda, junto con los 17 sub-bots.

**Problema de diseño a resolver antes de escribir un solo prompt:** el roster da un Input/Output orientativo por fila, pero no dice quién despacha a quién. Sin ese grafo explícito, cada prompt describiría su propio input/output de forma aislada y nadie podría verificar si conecta de verdad con el bot de al lado — exactamente el paso 8 de `plantilla_prompt.md` ("revisión cruzada... valida que el input/output realmente conecte"). Se diseñó el grafo completo antes de escribir nada, siguiendo el mismo patrón ya usado en Tech center (un hub con `dispatches_tasks = true` que despacha a especialistas, y algunos especialistas que a su vez despachan a otro especialista del mismo departamento sin volver a pasar por el hub — igual que Consultor de arquitectura despacha directo a Coder).

**Grafo de despacho de Upgrade & Review Center (Observar → Analizar → Mejorar):**

*Observar:*
- `Upgrade & review center` (cabeza) recibe de Efadam y despacha investigación/búsqueda cuando hace falta: `investigador`, `skill_finder_plataformas`, `skill_finder_generico`.
- `investigador` puede despachar a los skill finders para ampliar una plataforma o tema específico dentro de su propia investigación.
- `skill_finder_plataformas` y `skill_finder_generico` despachan sus hallazgos a `observador_patrones_replicables`.
- `abogado_scouter` corre de forma autónoma (igual que Ciber seguridad scouter en Tech center) monitoreando cambios legales; sus alertas alimentan a `cross_department` y, si hay una acción/contrato concreto de por medio, directo a `abogado_jefe`.
- `observador_patrones_replicables` y las alertas de `abogado_scouter` convergen en `cross_department`, el agregador interno de los 3 sub-clusters (Estrategia + Legal + Investigación) antes de reportar arriba.

*Analizar:*
- `cross_department` despacha su síntesis a `buscador_areas_oportunidad` y reporta también a la cabeza.
- `buscador_areas_oportunidad`, `automatizador`, `especialista_organizacion_metodos` y `out_of_the_box_thinker` generan ideas de forma independiente (cada uno con su propio input, ver su prompt) y todas convergen en `optimizador`, que las prioriza por costo/beneficio.
- `abogado_jefe` evalúa el riesgo legal de una acción/contrato propuesto (con las alertas de `abogado_scouter` como contexto) y despacha su dictamen a `abogado_verificador` cuando el resultado no es un rechazo — el veto de `abogado_jefe` no necesita doble check, el "procede" sí.

*Mejorar:*
- `optimizador` despacha sus ideas priorizadas a `council`, el punto de decisión final (gate humano vía Telegram, igual patrón que Abogado verificador).
- `council` decide: descarta, despacha a `establecer_metas` (si la idea es accionable dentro de metas ya existentes) o despacha a `nuevos_departamentos` (si implica crear un área/agente nuevo — esto además dispara el aviso humano obligatorio a Mateo que ya definía el prompt de la cabeza).
- `establecer_metas` fija la meta con plazo y despacha a `planner`, que arma el plan de acción concreto.
- `abogado_verificador` es el checkpoint humano final (Telegram) antes de que algo con implicación legal se ejecute de verdad — su aprobación libera la acción; su rechazo la corta ahí, sin pasar por Council.
- La cabeza consolida lo que sale de `planner` (plan de acción) y de `council` (decisiones tomadas en el periodo) y reporta a Efadam — igual patrón que Tech center y Proyect center.

Este grafo queda como referencia para escribir cada prompt (su "Input que recibe"/"Output que entrega" debe coincidir exactamente con esto) y para la prueba cruzada del paso 8 de `plantilla_prompt.md` una vez que los 17 estén escritos.

**Alcance de esta ronda: "escribir", no "activar".** Los ~17 bots se escriben con el mismo nivel de detalle que los 12 de Tech center (contrato operativo, JSON de despacho, casos de prueba), pero no se insertan en la tabla `bots` con `active = true` todavía. Activar 17 bots nuevos de una sola vez sin probar cada uno en vivo repetiría exactamente el error que ya se evitó con Tech center (activar sin probar) — la activación se hace en rondas siguientes, de a poco, con la misma disciplina de prueba en vivo antes de dar por cerrado cada uno. `86bbhawuq` se actualiza para reflejar este alcance y se abre una tarea separada para la fase de activación.

**Los 18 archivos quedaron escritos** (el bot cabeza migrado + los 17 sub-bots), siguiendo el grafo de arriba al pie de la letra — cada "Input que recibe"/"Output que entrega" de un prompt coincide exactamente con el destino declarado en el prompt del bot vecino, verificado archivo por archivo al escribirlos (no solo al diseñar el grafo):

- `prompts/estrategia-crecimiento/`: `upgrade-review-center.md` (cabeza, migrado desde la vision doc), `cross-department.md`, `buscador-areas-oportunidad.md`, `automatizador.md`, `especialista-organizacion-metodos.md`, `out-of-the-box-thinker.md`, `optimizador.md`, `council.md`, `nuevos-departamentos.md`, `establecer-metas.md`, `planner.md`.
- `prompts/investigacion-skills/`: `investigador.md`, `skill-finder-plataformas.md`, `skill-finder-generico.md`, `observador-patrones-replicables.md`.
- `prompts/legal/`: `abogado-scouter.md`, `abogado-jefe.md`, `abogado-verificador.md`.

**Decisiones de diseño que valieron la pena anotar aparte, porque no eran obvias del roster:**
- El mecanismo de aprobación humana real de este sistema (tabla `approvals` + `tasks.status = 'needs_approval'`, ya existente desde `001_init.sql`) se usa dos formas distintas según el caso, y ambas quedan documentadas explícitamente en cada prompt para que no se preste a confusión: (1) un bot razona y decide, y el GATE vive en la asignación que despacha (`requiere_aprobacion: true` en el siguiente salto) — así trabajan `Abogado Jefe` → `Abogado verificador` y `Council` → `Establecer metas`/`Nuevos departamentos`; (2) un bot es puramente un paso de formalización posterior a una aprobación ya resuelta (`Abogado verificador`, `Nuevos departamentos`) — su propio prompt asume que si está corriendo de verdad es porque el humano ya dijo que sí, y se dice explícito en "Estado y contrato operativo" para que no se le pida a ese bot que "decida" algo que ya se decidió.
- **Limitación conocida, documentada en `abogado-verificador.md`:** el mecanismo para RECHAZAR una fila de `approvals` (no solo aprobar) y reanudar la tarea hacia un estado terminal distinto no está construido todavía — mismo tipo de hueco ya documentado para Jarvis/reanudador. Se decidió documentarlo como limitación conocida en vez de inventar un mecanismo de rechazo que no existe.
- `Council`, `Establecer metas` y `Planner` respetan la regla ya establecida de que **los 3 centers nunca se reportan entre sí** — cuando un paso de un plan requiere ejecución de Tech center o Proyect center, `Planner` lo marca con `corresponde_a` pero no lo despacha directo; esa recomendación cruza por Efadam más adelante, igual que cualquier otra.
- `Establecer metas` y `Planner` de este departamento son instancias propias (decisión ya tomada en `roster_agentes_v4.xlsx`, no nueva de esta ronda) — sus prompts dejan explícito que Proyect center tendrá su propia copia con slugs distintos a definir en la activación (ej. `establecer_metas_estrategia` vs. `establecer_metas_proyectos`), para no chocar en la tabla `bots`.
- `Out of the box thinker` (único bot con autonomía total sobre su propio prompt) queda con una limitación honesta señalada en su propio archivo: el mecanismo real para aplicar un cambio que se proponga a sí mismo no está construido — hasta que exista, cualquier propuesta suya se trata como el código de Coder hoy, revisada por Mateo antes de mergear, nunca auto-aplicada.

**Pendiente explícito, no resuelto en esta ronda:** el paso 8 de `plantilla_prompt.md` pide revisión cruzada cuando hay más de una persona escribiendo prompts — no aplica literalmente (una sola sesión escribió los 18), pero sí vale la pena una relectura fresca cruzada en una ronda futura antes de activar, con la misma sospecha crítica que ya se aplicó a la auditoría externa. También queda pendiente decidir los slugs reales de `bots.slug` para los 18 (los nombres usados en los prompts son propuestas razonables, no definitivos) antes de insertarlos.
