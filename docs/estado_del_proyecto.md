...
Jarvis (endpoint humano planeado para mensajes y adjuntos) → Efadam en el
centro + 3 ramas. Efadam entrega recomendaciones a cada center y el center
despacha el trabajo de su departamento. Telegram permanece para aprobaciones y
alertas; no se configura como entrada conversacional de Efadam. Narrativa
completa en [[arquitectura_general]]; versión corta que leen los bots en
[[arquitectura]].

```
                    JARVIS (mensajes y adjuntos)
                              |
                           EFADAM
...
Un solo workflow que corre a cualquier bot leyendo su fila de `bots`. Incluye, funcionando: contexto de linaje, sistema de aclaración completo con reanudación bot-a-bot, extracción de patrones de fallo (`conocimiento_directo`), lectura de `tasks.esfuerzo` para elegir modelo, y **propagación de `esfuerzo` a tareas hijas** (18/ago — antes se perdía; ahora Técnico jefe y Trouble shooter heredan su propio nivel hacia lo que despachan, tal como lo define `efadam.md`).
...
**Operaciones — concepto nuevo, construido y probado en vivo el 18/ago, noche.** Tabla `operations` (el hilo de trabajo completo detrás de una tarea o grupo de tareas: una petición de usuario, una investigación, una ronda de autoexpansión) y `tasks.operation_id`, propagado por subquery SQL. Efadam abre la operación y entrega una recomendación al center correspondiente; el center decide y despacha el trabajo de su departamento. Hoy nada la usa de verdad porque Efadam no existe como bot activo, pero el schema y la propagación ya están listos. La síntesis de aprendizaje conserva el `operation_id`, produce una propuesta para U&R center y no bloquea la confirmación al cliente; su disparo automático sigue pendiente. Detalle completo en `ejecutor_generico.md`.
...
Como parte de probar todo esto en vivo se encontraron y repararon 2 tareas reales atoradas en el backlog (ids 4 y 7, desde el 13-14/ago) — les faltaba `esfuerzo`, quedaban atoradas en `running` por el mismo bug de arriba. Ya corrieron y quedaron `done` con salida real de modelo.
...
Departamento Dev/Tech: Prompt perfection, Entrenador Agentes, Coder, Agent builder, Trouble shooter, Ciber seguridad scouter, Hacker ético, Ciber seguridad, Técnico jefe, Tech center. Efadam (central, en `prompts/_core/`).
...
- **Modelo de esfuerzo:** Efadam recomienda un esfuerzo inicial por
  complejidad y preferencia de servicio (velocidad, equilibrio o rendimiento).
  Cada center clasifica de nuevo las tareas concretas. Riesgo y dominio activan
  controles de aprobación, pero no eligen por sí solos el modelo. La interfaz
  debe mostrar operaciones activas y permitir ajustar el esfuerzo, con
  historial y sin alterar tareas en ejecución. OmniRoute resuelve el esfuerzo
  al modelo real. Detalle completo
  en `stack_y_convenciones.md`, sección "Esfuerzo y BYOK".
  La migración `009_renombrar_esfuerzo.sql` y el export actualizado del
  Ejecutor genérico están listos en el repositorio, pero aún deben aplicarse
  a Postgres e importarse en n8n para quedar activos en la instancia.
