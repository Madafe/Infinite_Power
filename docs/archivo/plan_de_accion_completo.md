...
## Actualización — 19 de agosto de 2026, cuarta ronda (Mateo responde a las 3 preguntas de la ronda anterior, corrige el mecanismo de "tabla nueva" para nivel fijo, aclara Upgrade & review center, agrega lectura directa de Postgres para los 3 centers, y se cierra el pendiente 38) — VIGENTE, léase primero
...
34. **Refinado (19/ago, cuarta ronda) — diseño ya confirmado por Mateo en sus 3 partes, falta construir.** Workflow de aprobación humana bidireccional (C4), con dos rutas de escalamiento distintas (ver actualización del 19 de agosto, cuarta ronda, arriba, punto 3): Tipo A (aprobación obligatoria por regla de dominio) sube hasta el usuario vía `operations.bloqueada` + Efadam; Tipo B (duda de coordinación entre bots, sin regla de dominio de por medio) sube un nivel a la vez usando `tasks.blocked` + el "Reanudador de bloqueados" ya existente, sin llegar al usuario salvo que nadie en la cadena pueda resolverla. El mensaje final a usuario se compone con una plantilla obligatoria de campos (no con un modelo más caro — Mateo descartó la excepción de nivel `medio`, ver pregunta 2 de la misma actualización). Sigue pendiente, sin bloquear más decisiones de Mateo: construir esto en n8n (nuevo, no existe todavía), y correr `schema/008_bot_roles.sql` (tabla `bot_niveles_fijos`, ver punto 1 de la actualización) una vez que Mateo confirme la forma exacta de esa tabla.
35. **Re-explicado con ejemplo (19/ago, cuarta ronda) — sigue sin confirmar, Mateo pidió aclarar.** Ejemplo concreto: se abre una operación a nivel `bajo` — "investigar proveedores de hosting". Tiene sentido, es investigación normal. A la mitad, un bot decide que hay que contratar a un proveedor — eso es gasto de dinero, y las reglas de asignación dicen que gasto de dinero siempre es `crítico`, sin excepción. Pregunta: ¿esa tarea específica de "contratar al proveedor X" se queda en `bajo` (porque hereda el nivel fijo de la operación completa), o sube a `crítico` (porque las reglas de dominio lo exigen para esa tarea en particular)? Tal como quedó diseñado el 18 de agosto, el nivel se fija **una sola vez** al abrir la operación y las tareas hijas lo heredan — no hay mecanismo que lo suba aunque el contenido de una tarea específica lo amerite. Mi propuesta sigue siendo que no se quede fijo: que el nivel real de cada tarea sea el más alto entre el de la operación y el que le corresponda por su propio contenido (`max(nivel de la operación, nivel por reglas de asignación de esa tarea)`) — así una operación barata de investigación no se convierte en la forma de que un gasto de dinero real termine corriendo sin aprobación. Sigue pendiente de que Mateo confirme o proponga otra solución.
