...
14. ~~Decidir qué hacer con dos archivos sueltos sin commitear~~ — **hecho:** `_tmp_diag_github.ps1` (workflow ya borrado) y `_tmp_inspect_schedule.js` (exploración ya resuelta) se revisaron — sin secretos — y se borraron del disco. Nunca estuvieron trackeados, no generaron commit.
