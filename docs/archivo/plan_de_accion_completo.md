...
38. **Revisado (19/ago), queda una decisión de Mateo pendiente.** `docker-compose.yml`: los 3 servicios publican puerto sin restringir a localhost — `"5432:5432"` (Postgres), `"5678:5678"` (n8n), `"20128:20128"` (omniroute) — la sintaxis `"host:contenedor"` sin IP explícita hace bind a `0.0.0.0`, o sea accesibles desde cualquier interfaz de red de la máquina, no solo desde ella misma. Postgres es el más sensible (auth por contraseña, sin capa extra). **Pregunta para Mateo:** ¿accedes a Postgres/n8n/omniroute desde otra máquina en tu red (otra compu, el celular, algo tipo Tailscale)? Si no, se puede restringir a `"127.0.0.1:5432:5432"` (y análogo para los otros dos) sin perder nada — solo la propia máquina los alcanzaría. Si sí necesitas acceso remoto, no se toca esto y se documenta como aceptado. Imágenes sin pin de versión: `n8nio/n8n:latest` y `diegosouzapw/omniroute:latest` (`postgres:16` sí está razonablemente pinneado) — un `docker compose pull` puede saltar a una versión mayor rompiendo algo sin aviso; pineable a una versión específica cuando Mateo decida, no urgente mientras nadie corra `pull` a la ligera. CI/pruebas automatizadas: confirmado que no existe (`.github/` no existe, sin archivos de test en el repo) — pendiente real pero de fondo, no bloqueante mientras el proyecto es de una sola persona.
