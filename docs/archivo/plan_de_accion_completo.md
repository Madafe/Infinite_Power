...
## Actualización — 19 de agosto de 2026, sexta ronda (propuesta de diseño para el pendiente 37 — límites de fan-out/costo) — EN DISCUSIÓN, no decidido, VIGENTE, léase primero
...
34. **Decidido (19/ago, quinta ronda) — falta construir.** Workflow de aprobación humana bidireccional (C4), con dos rutas de escalamiento (Tipo A: aprobación obligatoria por regla de dominio, sube hasta el usuario vía `operations.bloqueada` + Efadam; Tipo B: duda de coordinación entre bots, sube un nivel a la vez vía `tasks.blocked` + "Reanudador de bloqueados", nunca llega al usuario una pregunta que era para un bot intermedio — ver actualización del 19 de agosto, cuarta ronda, punto 3). Mensaje final a usuario compuesto con plantilla obligatoria de campos, no con modelo más caro. `bot_niveles_fijos` ya corrida contra Postgres (ver actualización del 19 de agosto, quinta ronda) — falta poblarla y falta construir la lógica en n8n (nuevo, no existe todavía).
...
37. **Refinado (19/ago, sexta ronda) — propuesta en discusión, no decidido.** Ver actualización del 19 de agosto, sexta ronda, arriba, para el diseño completo. Resumen: tope de fan-out por despacho (N tareas por pasada) y por operación (COUNT total), usando el mismo mecanismo Tipo A de bloqueo/escalamiento del pendiente 34, sin tablas nuevas. El tope de costo usa `agent_runs.cost_estimate` (ya existe en el schema) pero está bloqueado hasta que el Paso 6 (Monitoreo/costos) exista de verdad — hoy `agent_runs` no se llena. Quedan 2 preguntas para Mateo: si los topes varían por `nivel_importancia`, y si se adelanta la construcción del Paso 6 para destrabar el tope de costo.
